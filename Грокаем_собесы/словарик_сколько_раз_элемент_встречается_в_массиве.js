...
const countElements = (array) => {
  return array.reduce((objCount, element) => {
    // изнчально у нас "objCount" - пустой объект {},так как это "аккамулятор" (сделали его "инишл-значение",что вернём из reduce как {}),
    // и мы обращаемся к его свойству (получаем значение своиства): objCount[element] - при первой итерации это: objCount['kiwi']
    // оператор "+" главнее чем "=", поэтому читается (выполняется) сначала выражение справа от знака "равно";
    // objCount[element] - это значение свойства объекта,если значения нет,как у нас изначально {} т.е objCount[element] нету,
    // значит результат "первой" итерации
    ...
    */
    ...
    return objCount // из reduce всегда возвращаем итоговый счетчик, а сам новый массив возвращаем из функции
  }, {}) // "инишл" acc === objCount -- аккамулятор в редюсе идет 1 параметром, потом элемент массива итерации, потом index, array
}
